...
- **Repeatable logging.** Tapping an achievement logs it again — every log adds its points again, with a 1×/2×/3× count badge on the row and a − button to take the latest one back off.
- **School bonus.** Any achievement can be marked as allowing the school bonus when it's created or suggested. Logging one asks a single question — add 1,000 bonus points for doing it with someone from school, or log it plain (no custom numbers); the bonus shows in the live feed, toasts, and score totals.
- **Live feed and notifications.** On desktop the game sits on the left with a sticky live-activity feed on the right; each entry shows the repeat count and the player's running total. On phones the feed sits under the leaderboard. Everyone gets an in-app toast when another player logs something, plus optional browser notifications via the bell in the feed.
- **Reopenable finalize.** The creator can reopen the finalized list at any time: logging pauses for everyone while it's open (points and history are untouched), and finalizing again resumes the game.
- **Two tabs in every phase.** *Leaderboard* lists every player with their profile picture — while the list is being drafted it is a roster with everyone on zero, newest joiner first; once finalized it ranks by points and gains a recent-activity feed. Tapping a player opens their profile and every achievement they've ticked off. *Achievements* is the draft/approval list before finalizing and a tick-and-untick checklist after — logs are repeatable with count badges, and the − button removes one. The tab shown by default follows the phase.
