...
(function () {
  ...
  function App() {
    ...
    var _e = useState(null), myId = _e[0], setMyId = _e[1];
    ...
    async function loadAll(isRefresh) {
      ...
      if (failed) {
        ...
        if (me.ok && me.value && me.value.playerId) setMyId(me.value.playerId);
        setStatus('ready');
    ...
    useEffect(function () { loadAll(false); }, []);

    /* Firebase pushes changes live — subscribe once and mirror into state */
    useEffect(function () {
      if (MODE !== 'firebase' || !FB) return;
      function parse(v) {
    ...
    async function createGame(form) {
      return withBusy(async function () {
        ...
        var pr = await saveProfile({ id: playerId, name: form.name.trim(), avatar: form.avatar, contact: form.contact }, true);
    ...
    async function joinGame(form) {
      return withBusy(async function () {
        ...
        /* same email/phone as an existing player -> sign back into that profile
    ...
    if (!game) {
      return html`<div className="shell">
        ${headerBar}${banners}
        <${SetupScreen} busy=${busy} onCreate=${createGame} />
      </div>`;
    }

    /* game exists but this device hasn't joined */
    if (!me) {
      return html`<div className="shell">
        ${headerBar}${banners}
        <${JoinScreen} busy=${busy} phase=${game.phase} players=${players} onJoin=${joinGame} />
      </div>`;
    }
    ...
    return html`<div className="shell">
      ...
      ${editingProfile ? html`<${Sheet} onClose=${function () { setEditingProfile(false); }}>
        <${EditProfile} me=${me} busy=${busy} onSave=${updateMyProfile} />
      <//>` : null}
    ...
    if (step === 1) {
      return html`<main className="card">
        ...
        <p className="muted">No game exists here yet. Whoever sets it up becomes the <strong>game creator</strong> —
          the only person who can approve achievements and finalize the list. Sign up to get started.</p>
        <${ContactField} value=${contactRaw} error=${contactErr} onChange=${setContactRaw} />
        <label className="field">
    ...
    if (step === 1) {
      return html`<main className="card">
        ...
        ${props.phase === 'play'
          ...
          : html`<p className="muted">The group is still drafting achievements. Sign up to suggest your ideas.</p>`}
        <p className="muted">Already signed up on another device? Enter the same email or number to get your profile back.</p>
        <label className="field">
  ...
  function EditProfile(props) {
    ...
    return html`<div>
      ...
      ${props.me.contact ? html`<div className="contact-line">
        <span className="field-label">Signed up with</span>
